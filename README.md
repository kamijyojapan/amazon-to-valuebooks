...
Amazon.co.jp の書籍ページを開いた際に、古本買取・販売サイト「バリューブックス」の在庫状況と価格を自動で検索し、結果を通知バーで表示する UserScript です。
...
Greasy Fork からインストール (※公開後にリンクを貼ってください)

または、GitHubのファイル一覧にある .user.js をクリックして「Raw」ボタンを押すとインストール画面が開きます。

🚀 使い方

Amazon.co.jp で任意の書籍（紙の本、Kindle問わず）の詳細ページを開きます。

ページ読み込み後、商品タイトルの下あたりに通知バーが表示されます。

バーをクリックすると、バリューブックスの商品ページが新しいタブで開きます。
...
タイトルの正規化: (文庫) [Kindle版] などの不要な文字列を削除しますが、コミックの巻数 (12) などは保持します。
...
類似度判定: ヒットした商品のタイトルとAmazonのタイトルを比較（レーベンシュタイン距離）し、全く違う商品が誤って表示されるのを防ぎます。
